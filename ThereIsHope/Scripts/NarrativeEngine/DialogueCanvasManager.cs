...
public class DialogueCanvasManager : MonoBehaviour
{
    ...
    public Diary_Controller dController;
                    ...
                    foreach (DialogueChoice nextChoice in nEngine.currentInteraction.DialogueChoices)
                    {
                        if (nextChoice.ID == nextID)
                        {
                            ...
                            if (nextChoice.TriggerTag != "")
                            {
                                ...
                                {
                                    case "Bully":
                                        foreach (GameObject g in GameObject.FindGameObjectsWithTag(nextChoice.TriggerTag))
                                        {
                                            Enemy_Controller tempBullyController = g.GetComponent<Enemy_Controller>();
                                            tempBullyController.dialogueTrigger = true;
                                        ...
                                        break;
                                    case ("Jack"):
                                        JackController tempJackController = GameObject.FindGameObjectWithTag("Jack").GetComponent<JackController>();
                                        tempJackController.dialogueTrigger = true;
                                        break;
                                    case ("Complete"):
                                        a1Manager.isComplete = true;
                                        break;
                                }
                            }
                        }
